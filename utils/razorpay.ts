...
const RAZORPAY_KEY_ID = 'rzp_live_RrozxPwaxxlYaP'; // From your .env
...
export class RazorpayService {
  ...
  static async verifyPayment(
    ...
    try {
      ...
      const response = await fetch('https://api.mentoservices.com/api/v1/subscription/verify', {
        method: 'POST',
        headers: {
  ...
  static async processPayment(planId: string, planName: string, amount: number): Promise<boolean> {
    try {
      ...
      const createResponse = await fetch(
        `https://api.mentoservices.com/api/v1/subscription/create/${planId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
